...
export default async function AdminSpecialistsPage() {
  const requests = await prisma.specialistConsultation.findMany({
    ...
    <div className="min-h-screen bg-[#fafafa] p-8 md:p-12">
      <header className="mb-12 flex items-center justify-between">
        <div>
           ...
           <h1 className="text-4xl font-black text-zinc-900 tracking-tighter">Specialist Queue</h1>
        </div>
        <div className="flex items-center gap-2 bg-zinc-900 text-white px-4 py-2 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-zinc-900/20">
           ...
           {requests.length} Consultations
        </div>
      </header>

      <SpecialistTable initialRequests={requests} />
    </div>
  );
}
